...
module.exports.generate_transcoder=function (o) {
    ...
    if (o.cfg_transcode.indexOf('flac')>-1) {
        ...
        p=p.on('activate','', (verb,cd) =>  { o.flac_transcoder.activate(`${verb} flac`,cd); })
            .on('process','track',(idx,input) =>  { return o.flac_transcoder.process_track(idx,input); })
            .on('process','backlog',(idx,input)=> { o.flac_transcoder.cb_process_backlog(idx,input); });
            ...
            return new Promise((resolve,reject)=>{
                mkdirp(cd.album_path,(err)=>{
                    if (err) { reject(err);}
                        ...
                        //flac_transcoder.cb_start_backlog(1,0);
                        resolve(cd);
                    }
                });
            });  
